...
// writeMeshkitErrorWithCtx is like WriteMeshkitError but accepts a request ID
// directly. It is used internally and by the handler wrapper.
func writeMeshkitErrorWithCtx(w http.ResponseWriter, err error, status int, reqID string) {
	encodeError(w, err, status, reqID)
}
